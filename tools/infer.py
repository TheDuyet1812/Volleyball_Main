...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--best_check_point', type=str, default='weights/best_checkpoint.model', help='model path(s) for inference.')
...
def run(weights=osp.join(ROOT, 'yolov6s.pt'),
        ...
        hide_conf=False, 
        half=False,
        best_check_point=osp.join(ROOT, 'best_checkpoint.model'),
        ):
    ...
    # Inference
    inferer = Inferer(source, webcam, webcam_addr, weights, device, yaml, img_size, half, best_check_point)
