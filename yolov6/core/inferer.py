...
from yolov6.utils.events import LOGGER, load_yaml, ConvNet
...
class Inferer:
    def __init__(self, source, webcam, webcam_addr, weights, device, yaml, img_size, half, best_check_point):
        ...
        self.best_check_point = best_check_point

        cuda = self.device != 'cpu' and torch.cuda.is_available()
        self.device = torch.device(f'cuda:{device}' if cuda else 'cpu')
        self.checkpoint= torch.load(self.best_check_point, map_location=self.device)
     
        self.model_classify=ConvNet(num_classes= len(self.class_names)) 
        self.model_classify.load_state_dict(self.checkpoint)
    ...
    def Count_volleyball(self,lst, det, count, lis, x):
      if len(lst)!= 0:
        # self.n = min(len(det[:,5]), len(lst[0]))
        
        self.z  =  torch.unique(lst[0])
        for i in self.z:
            if i != 0:
                if len(lis)!= 0:
                    if i != lis[0]:
                        ...
                        x[0] = 0
                lis.append(i)
            if len(self.z) == 1 and len(lis) >3:
                x[1]+= 1
                
                 
        if len(lis)>= 8 and x[0] >= 3 :
            lis = lis[(-x[0]):]
            count+= 1
            x[1]= 0
            
            
        if len(lis)>=3 and x[1]>=15: ##
          lis =[]
          x[1]= 0
          count+= 1
            ...
            if len(det): 
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        x_min = int((x_center - (width / 2)) * img_crop.shape[1])
                        y_min = int((y_center - (height / 2)) * img_crop.shape[0])
                        x_max = int((x_center + (width / 2)) * img_crop.shape[1])
                        y_max = int((y_center + (height / 2)) * img_crop.shape[0])
                        
                        crop = img_crop[y_min:y_max, x_min:x_max][:,:,[2,1,0]]
                        ...
                        transformer=transforms.Compose([
                            ...
                            transforms.Normalize([0.5,0.5,0.5], # 0-1 to [-1,1] , formula (x-mean)/std
                                                [0.5,0.5,0.5])])
                        crop_tensor = Image.fromarray(crop)
                        image_tensor=transformer(crop_tensor).float()
                        ...
                        output=self.model_classify(input)
                        # print(output)
                        
                    
                        index=output.data.numpy().argmax() 
                        # ten = str(index)+'_'+ str(x) +'.jpg' 
                        # savefolder ='/content/gdrive/MyDrive/YOLOv6/anh'
                        # cv2.imwrite(os.path.join(savefolder, ten), crop)
                        # x+=1
                    ...
                    if save_img:  
                        ...
                        self.plot_count(img_ori, self.count) ###

                img_src = np.asarray(img_ori)
            self.count, self.lst, self.lis, self.x = self.Count_volleyball(self.lst, det, self.count, self.lis, self.x)
            print(self.count)
            # print(self.lst) 
            # print(self.count) 
    ...
    def plot_count(self, image, count):
        count = str(count) 
        w, h, i = image.shape  
        h = h-50 
         
        cv2.putText(image, count, (h, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        # tf = max(lw - 1, 1)  # font thickness
        
        # cv2.putText(image, count, (0,0), font, lw / 3, txt_color,
        #                 thickness=tf, lineType=cv2.LINE_AA)
